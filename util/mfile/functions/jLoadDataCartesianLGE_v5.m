% --------------------------------------------------------------------------------
% [Data]=jLoadDataCartesianLGE_v5(FileInfo,Mode,UndersampleFactor)
% Jason Mendes - UCAIR - University of Utah - 2017
% --------------------------------------------------------------------------------
% Load data from measurement file.
% --------------------------------------------------------------------------------
function [Data]=jLoadDataCartesianLGE_v5(FileInfo,Mode,UndersampleFactor)
...
if (exist('UndersampleFactor','var')~=1)
    UndersampleFactor=1.0;
end
Data=[];
N=length(FileInfo.Protocol);
Data.Info.TotalScanTime=FileInfo.Protocol(N).lTotalScanTimeSec;
Data.Info.InversionTime=FileInfo.Protocol(N).alTI/1e3;
Data.Info.TimePerShot=FileInfo.Protocol(N).alTR(1)/1e3;
Data.Info.LinesPerShot=FileInfo.Protocol(N).sFastImaging.lSegments;
Data.Info.EchoTime=FileInfo.Protocol(N).alTE(1)/1e3;
Data.Info.FlipAngle=FileInfo.Protocol(N).adFlipAngleDegree;
Data.Info.Partitions=FileInfo.Protocol(N).sKSpace.lPartitions;
Data.Info.PhaseEncodeLines=FileInfo.Protocol(N).sKSpace.lPhaseEncodingLines;
Data.Info.SliceThickness=FileInfo.Protocol(N).sSliceArray.asSlice.dThickness;
Data.Info.ReadoutFOV=FileInfo.Protocol(N).sSliceArray.asSlice.dReadoutFOV;
Data.Info.PhaseFOV=FileInfo.Protocol(N).sSliceArray.asSlice.dPhaseFOV;
% When GRAPPA is on there is a change in line numbers not relected in the ASCCONV section
Header=jGetHeader(FileInfo,5);
if (Data.Info.PhaseEncodeLines<(2*Header.KspaceLineCenter))
    Data.Info.PhaseEncodeLines=2*Header.KspaceLineCenter;
end
Data.Info.ReadoutSamples=FileInfo.Protocol(N).sKSpace.lBaseResolution;
Data.Info.NumberOfChannels=FileInfo.Protocol(N).lNumberOfChannels;
Data.Info.DataReadMode=Mode;
Data.Info.DataReadUndersampleFactor=UndersampleFactor;
Data.Kspace=single(complex(0,0));
Data.Kspace(Data.Info.ReadoutSamples,Data.Info.PhaseEncodeLines,Data.Info.Partitions,Data.Info.NumberOfChannels)=single(complex(0,0));
Data.Kspace(:)=single(complex(0,0));
Data.KspaceMask=zeros(Data.Info.ReadoutSamples,Data.Info.PhaseEncodeLines,Data.Info.Partitions,'single');
TempData=single(complex(0,0));
TempData(2*Data.Info.ReadoutSamples,1,1,Data.Info.NumberOfChannels)=single(complex(0,0));
TempIndex=(1:Data.Info.ReadoutSamples)-floor(Data.Info.ReadoutSamples/2)+floor(Data.Info.ReadoutSamples);

% Determine navigator location
if (~strcmpi(Mode,'safemode'))
    NavigatorLine=zeros(1,3*Data.Info.LinesPerShot+6,'single');
    for LineIndex=1:(3*Data.Info.LinesPerShot+6)
        Header=jGetHeader(FileInfo,LineIndex);
        NavigatorLine(LineIndex)=Header.RealTimeFeedBack;
    end
    if (min(diff(find(NavigatorLine>0)))<Data.Info.LinesPerShot)
        Data.Info.NavigatorPosition='BeforeAndAfterShot';
        BlockSize=Data.Info.LinesPerShot+2;
        DataLocation=1+(1:Data.Info.LinesPerShot);
    elseif NavigatorLine(2*(Data.Info.LinesPerShot+1))
        Data.Info.NavigatorPosition='AfterShot';
        BlockSize=Data.Info.LinesPerShot+1;
        DataLocation=1:Data.Info.LinesPerShot;
    elseif NavigatorLine(2*(Data.Info.LinesPerShot+1)+1)
        Data.Info.NavigatorPosition='BeforeShot';
        BlockSize=Data.Info.LinesPerShot+1;
        DataLocation=1+(1:Data.Info.LinesPerShot);
    else
        Data.Info.NavigatorPosition='Unknown';
        BlockSize=Data.Info.LinesPerShot;
        DataLocation=1:Data.Info.LinesPerShot;
    end
end

% Determine total read time based on data
Header=jGetHeader(FileInfo,1);
StartTime=Header.Time;
Header=jGetHeader(FileInfo,length(FileInfo.Protocol(N).OffsetImage));
FinishTime=Header.Time;
Data.Info.AcquireTime=FinishTime-StartTime;

% Safe mode read if require
if (strcmpi(Mode,'safemode'))
    for LineIndex=1:length(FileInfo.Protocol(N).OffsetImage)
        [ComplexData,Header]=jGetDataAndHeader(FileInfo,LineIndex);
        if (~Header.RealTimeFeedBack)
            Ky=Header.Line-Header.KspaceLineCenter+floor(Data.Info.PhaseEncodeLines/2)+1;
            Kz=Header.Partition-Header.KspacePartitionCenter+floor(Data.Info.Partitions/2)+1;
            Kx=(1:Header.Samples)-Header.Samples+2*Data.Info.ReadoutSamples;
            TempData(Kx,1,1,:)=ComplexData;
            TempData=fftshift(ifft(ifftshift(TempData,1),[],1),1);
            Data.Kspace(:,Ky,Kz,:)=fftshift(fft(ifftshift(TempData(TempIndex,1,1,:),1),[],1),1);
            Data.KspaceMask(:,Ky,Kz)=1;
        end
    end
    if (CloseFileOnExit)
...
% Read in Kspace data
Data.Info.NumberOfShots=floor(length(FileInfo.Protocol(N).OffsetImage)/BlockSize);
CurrentLine=zeros(1,Data.Info.LinesPerShot);
NextLine=zeros(1,Data.Info.LinesPerShot);
CurrentPartition=zeros(1,Data.Info.LinesPerShot);
NextPartition=zeros(1,Data.Info.LinesPerShot);
Data.Info.UndersampledShots=max(min(ceil(Data.Info.NumberOfShots*UndersampleFactor),Data.Info.NumberOfShots),1);
for BlockIndex=1:Data.Info.UndersampledShots
    if (BlockIndex==1)
        for LineIndex=1:length(DataLocation)
            [Header]=jGetHeader(FileInfo,(BlockIndex-1)*BlockSize+DataLocation(LineIndex));
            CurrentLine(LineIndex)=Header.Line;
            CurrentPartition(LineIndex)=Header.Partition;
        end
    else
        CurrentLine=NextLine;
        CurrentPartition=NextPartition;
    end
    if (BlockIndex<Data.Info.NumberOfShots)
        for LineIndex=1:length(DataLocation)
            [Header]=jGetHeader(FileInfo,BlockIndex*BlockSize+DataLocation(LineIndex));
            NextLine(LineIndex)=Header.Line;
            NextPartition(LineIndex)=Header.Partition;
        end
    else
        NextLine=zeros(1,Data.Info.LinesPerShot);
        NextPartition=zeros(1,Data.Info.LinesPerShot);
    end
    if (~((prod(CurrentLine==NextLine))&&(prod(CurrentPartition==NextPartition))))
        fprintf('Adding block %d of %d\n',BlockIndex,Data.Info.NumberOfShots);
        for LineIndex=DataLocation
            [ComplexData,Header]=jGetDataAndHeader(FileInfo,LineIndex+(BlockIndex-1)*BlockSize);
            if (~Header.RealTimeFeedBack)
                Ky=Header.Line-Header.KspaceLineCenter+floor(Data.Info.PhaseEncodeLines/2)+1;
                Kz=Header.Partition-Header.KspacePartitionCenter+floor(Data.Info.Partitions/2)+1;
                Kx=(1:Header.Samples)-Header.Samples+2*Data.Info.ReadoutSamples;
                if (((strcmpi(Mode,'noaverage'))&&(max(Data.KspaceMask(:,Ky,Kz))<1))||(~strcmpi(Mode,'noaverage')))
                    TempData(Kx,1,1,:)=ComplexData;
                    TempData=fftshift(ifft(ifftshift(TempData,1),[],1),1);          
                    Data.Kspace(:,Ky,Kz,:)=Data.Kspace(:,Ky,Kz,:)+fftshift(fft(ifftshift(TempData(TempIndex,1,1,:),1),[],1),1);
                    Data.KspaceMask(:,Ky,Kz)=Data.KspaceMask(:,Ky,Kz)+1;
                end
            end
            FinishTime=Header.Time;
        end
    else
        fprintf('Skipping block %d of %d\n',BlockIndex,Data.Info.NumberOfShots);
    end
end
RealDataSum=sum(sum(max(Data.KspaceMask,[],1),2),3);
Weight=min(1./(Data.KspaceMask+(Data.KspaceMask==0)),1);
Data.Kspace=Data.Kspace.*repmat(Weight,[1 1 1 Data.Info.NumberOfChannels]);
Data.KspaceMask=Data.KspaceMask.*Weight;
DataSum=sum(sum(max(Data.KspaceMask,[],1),2),3);
Data.Info.RealR=(Data.Info.PhaseEncodeLines*Data.Info.Partitions)/max(RealDataSum,1);
Data.Info.R=(Data.Info.PhaseEncodeLines*Data.Info.Partitions)/max(DataSum,1);
Data.Info.AcquireTimeWithFactor=FinishTime-StartTime;
...
elseif (FileInfo.IsVD)
   ...
   Header.GainSwitch=min(bitand(Header.Mask(1),2^10),1);
   Header.CalibrateKWIC=min(bitand(Header.Mask(2),2^3),1);
   Header.RealTimeFeedBack=min(bitand(Header.Mask(1),2^1),1);
