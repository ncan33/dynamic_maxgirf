function [kernel, kernel_im_space] = SSG_train_section(ks, para)
...
%         idx = (1:npatch) + (j-1)*npatch;
        source_temp = get_patch_section_sl1(ks(:,:,:,j,i), ps);
        npatch = size(source_temp, 2);
        % the training aims to noun the aliaed k-space data, so the source
        % is zero 
        if i==j
            target_temp = get_patch_center_section_sl1(kt(:,:,:,i), ps);
        else
            target_temp = zeros([nc, npatch], 'like', ks);
%             source_temp = 3 * get_patch_section_sl1(ks(:,:,:,j,i), ps);
        end
