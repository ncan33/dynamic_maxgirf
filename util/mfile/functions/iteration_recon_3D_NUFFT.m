function [Image,para] = iteration_recon_3D_NUFFT(Data,para)
...
%%%%% fidelity term/temporal/spatial TV
    ...
    fidelity_update(:,:,end-1:end,:,:) = 0;
    for i=1:length(Data.N)
        temp = NUFFT.NUFFT_new(squeeze(fidelity_update(:,:,i,:,:)),Data.N{i});
        fidelity_update(:,:,i,:,:) = NUFFT.NUFFT_adj_new(temp,Data.N{i});
    end
...
%%%%% plot&save part 
    ...
    if mod(iter_no,save_frequency)==0
        ...
        if debug
            save([save_dir_temp,'.mat'],'temporalNorm','spatialNorm','fidelityNorm','totalpara.Cost','-append');
        end
    end
    toc(t1);
end
...
%if debug
%    para.Cost = gather(para.Cost);
%    save([save_dir,'.mat'],'para.Cost','-append')
%end
