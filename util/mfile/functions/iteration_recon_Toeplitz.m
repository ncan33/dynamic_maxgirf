function [Image,para] = iteration_recon_Toeplitz(first_est,kSpace,mask,sens_map,Apodizer,para,varargin)
...
%%%%% fidelity term/temporal/spatial TV
    ...
    tic;
    if ifGPU
        fidelity_update = single(zeros(size(kSpace),'gpuArray'));
    else
        fidelity_update = single(zeros(size(kSpace)));
    end
    fidelity_update(l_cut+1:l_cut+l_i,l_cut+1:l_cut+l_i,:,:) = bsxfun(@times,new_img_x,sens_map);
    ...
    fidelity_update(1:l_cut,:,:,:) = [];
    fidelity_update(:,1:l_cut,:,:) = [];
    fidelity_update(end-l_cut+1:end,:,:,:) = [];
    fidelity_update(:,end-l_cut+1:end,:,:) = [];
    fidelity_update = bsxfun(@times,fidelity_update,sens_map_conj);
    fidelity_update = sum(fidelity_update,4);
    fidelity_update = bsxfun(@times,fidelity_update,Apodizer);
