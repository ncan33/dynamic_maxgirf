function [Image,para] = iteration_recon_3D_GPU_test(first_est,kSpace,sens_map,para)
...
%%%%% fidelity term/temporal/spatial TV
    ...
    kSpace_update(:,:,:,:,1:4) = fft2(kSpace_update(:,:,:,:,1:4));
    kSpace_update(:,:,:,:,5:8) = fft2(kSpace_update(:,:,:,:,5:8));
    kSpace_update(:,:,:,:,1:4) = fft(kSpace_update(:,:,:,:,1:4),[],3);
    kSpace_update(:,:,:,:,5:8) = fft(kSpace_update(:,:,:,:,5:8),[],3);
    fidelity_update = kSpace - kSpace_update; clear kSpace_update;
    fidelity_update = bsxfun(@times,fidelity_update,mask);
    fidelity_update(:,:,:,:,1:4) = ifft2(fidelity_update(:,:,:,:,1:4));
    fidelity_update(:,:,:,:,5:8) = ifft2(fidelity_update(:,:,:,:,5:8));
    fidelity_update(:,:,:,:,1:4) = ifft(fidelity_update(:,:,:,:,1:4),[],3);
    fidelity_update(:,:,:,:,5:8) = ifft(fidelity_update(:,:,:,:,5:8),[],3);
    fidelity_update = sum(bsxfun(@times,fidelity_update,conj(sens_map)),para.Recon.kSpace_dim);

    parfor i=1:4
        kSpace_update(:,:,:,:,:,i) = bsxfun(@times,new_img_x,sens_map(:,:,:,:,:,i));
        kSpace_update(:,:,:,:,:,i) = fft3(kSpace_update(:,:,:,:,:,i));
        kSpace_update(:,:,:,:,:,i) = bsxfun(@times,kSpace_update(:,:,:,:,:,i),mask);
        kSpace_update(:,:,:,:,:,i) = ifft3(kSpace_update(:,:,:,:,:,i));
        kSpace_update(:,:,:,:,:,i) = bsxfun(@times,kSpace_update(:,:,:,:,:,i),conj(sens_map(:,:,:,:,:,i)));
    end
    %}
    ...
    for i=1:prod(siz(4:end))
        fidelity_update(:,:,:,i) = fftn(fidelity_update(:,:,:,i));
    ...
    fidelity_update = reshape(fidelity_update,[siz(1:3),prod(siz(4:end))]);
    for i=1:prod(siz(4:end))
        fidelity_update(:,:,:,i) = ifftn(fidelity_update(:,:,:,i));
    end
    fidelity_update = reshape(fidelity_update,siz);
    ...
    fidelity_update = first_est - fidelity_update;
    fidelity_update = fidelity_update * mean(abs(new_img_x(:)));
...
%{
     update_term  = fidelity_update + tTV_update + sTV_update + sliceTV_update; clear tTV_update sTV_update sliceTV_update
     n = 1;
     step_size = 1;
         cost_old = sum(abs(new_img_x(:) - first_est(:)).^2);
         cost_old = cost_old + weight_tTV * sum(sum(sum(sum(abs(diff(new_img_x,4))))));
         cost_old = cost_old + weight_sTV * sum(sum(sum(sum(abs(diff(new_img_x,1)).^2))));
         cost_old = cost_old + weight_sTV * sum(sum(sum(sum(abs(diff(new_img_x,2)).^2))));
         cost_old = cost_old + weight_slTV * sum(sum(sum(sum(abs(diff(new_img_x,3)).^2))));
         
         cost_new = sum(abs(new_img_x(:) + step_size*update_term(:) - first_est(:)).^2);
         cost_new = cost_new + weight_tTV * sum(sum(sum(sum(abs(diff(new_img_x + step_size*update_term,4))))));
         cost_new = cost_new + weight_sTV * sum(sum(sum(sum(abs(diff(new_img_x + step_size*update_term,1)).^2))));
         cost_new = cost_new + weight_sTV * sum(sum(sum(sum(abs(diff(new_img_x + step_size*update_term,2)).^2))));
         cost_new = cost_new + weight_slTV * sum(sum(sum(sum(abs(diff(new_img_x + step_size*update_term,3)).^2))));
         cost_old
         cost_new
     while n<11 && cost_old>cost_new
         cost_old = cost_new;
         n = n+1;
