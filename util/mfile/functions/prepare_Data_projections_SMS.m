function [Data,para] = prepare_Data_projections_SMS(kSpace_all,RayPosition,para)
...
[Data.G,Data.kSpace] = GROG_seperate_SMS_GNUFFT(squeeze(kSpace_radial),kx,ky,para,phase_mod);

if nSMS == 1 
    para.Recon.type = '2D';
else
    para.Recon.type = 'seperate SMS projections less memory';
end
...
Data.phase_mod = reshape(phase_mod,[1,1,nor*nof,1,nSMS]);
Data.kSpace = ifft2(Data.kSpace);
Data.kSpace = fftshift2(Data.kSpace);
Data.kSpace = fft2(Data.kSpace);
Data.kSpace = Data.kSpace .* Data.mask;

im = ifft2(Data.kSpace.*conj(Data.phase_mod));
%im = fftshift2(im);
...
im = ifft2(Data.kSpace.*Data.filter.*conj(Data.phase_mod));
...
function [G,kSpace_cart] = GROG_seperate_SMS_GNUFFT(kSpace_radial, kx, ky, para, phase_mod)
...
kSpace_cart = zeros(sx+core_size(1)*2,sx+core_size(2)*2,nor*nof,nc,'single');
for j=1:nSMS
    for i=1:NSlice
        ...
        G{j} = GNUFFT_init(kSpace_radial_temp,kx_temp,ky_temp,over_sampling,core_size);
        kSpace_cart_temp = GROG.GNUFFT_rad2cart(kSpace_radial_temp,G{j});

        SMS_temp = SMS(:,:,:,:,:,j);
        SMS_temp = SMS_temp(:);
        kSpace_cart(:,:,SMS_temp,:) = kSpace_cart_temp;
    end
end
