...
% --------------------------------------------------------------------------------
function TriggerTime=jGetTriggerTimes(FileInfo)
...
NumberOfMeasurements=FileInfo.Protocol(N).lRepetitions+1;
NumberOfLines=length(FileInfo.Protocol(N).OffsetImage);
PositionFirstLine=(0:(NumberOfMeasurements-1))*NumberOfLines/NumberOfMeasurements+1;
TriggerTime=zeros(1,NumberOfMeasurements);
for MeasIndex=1:NumberOfMeasurements
    Header=jGetHeader(FileInfo,round(PositionFirstLine(MeasIndex)));
    TriggerTime(MeasIndex)=Header.Time;
end
