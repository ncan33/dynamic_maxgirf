...
% --------------------------------------------------------------------------------
% Temp file to load data
% --------------------------------------------------------------------------------
function [Data2D]=jLoadDataUCAIR_Temp(FileName)

% Open file
FileInfo=jGetFileInfo(FileName);
N=length(FileInfo.Protocol);
Data2D.Info.AcquireTime=FileInfo.Protocol(N).alTR(1)/1e3;
Data2D.Info.RepetitionTime=FileInfo.Protocol(N).alTR(2)/1e3;
Data2D.Info.ImageTime=FileInfo.Protocol(N).alTR(3)/1e3;
Data2D.Info.SaturationRecoveryTime=FileInfo.Protocol(N).alTI(1)/1e3;
Data2D.Info.ProtonDensityScans=FileInfo.Protocol(N).sWipMemBlock.alFree(11);
Data2D.Info.ProtonDensityFlipAngle=FileInfo.Protocol(N).sWipMemBlock.alFree(12);
Data2D.Info.EchoTime=FileInfo.Protocol(N).alTE(1)/1e3;
...
Data2D.Info.Multiband=FileInfo.Protocol(N).sWipMemBlock.alFree(13);
Data2D.Info.NumberOfSlices=length(FileInfo.Protocol(N).sSliceArray(1).asSlice);
Data2D.Info.MultiBandSliceSets=Data2D.Info.NumberOfSlices/Data2D.Info.Multiband;
Data2D.Info.NumberOfMeasurements=FileInfo.Protocol(N).lRepetitions+1;
Data2D.Info.LinesPerMeasurement=FileInfo.Protocol(N).sKSpace.lRadialViews;
Data2D.Kspace=complex(0,zeros(Data2D.Info.ReadoutSamples,Data2D.Info.LinesPerMeasurement,Data2D.Info.NumberOfMeasurements,Data2D.Info.MultiBandSliceSets,Data2D.Info.NumberOfChannels,'single'));
Data2D.Theta=zeros(Data2D.Info.LinesPerMeasurement,Data2D.Info.NumberOfMeasurements,Data2D.Info.MultiBandSliceSets,'single');
%Data2D.PhaseRF=ones(Data2D.Info.LinesPerMeasurement,Data2D.Info.NumberOfMeasurements,Data2D.Info.MultiBandSliceSets,'single')*3;
Data2D.TriggerTime=zeros(1,Data2D.Info.NumberOfMeasurements,'single');
% adding Ye 03/09/18
Data2D.LineType = ones(Data2D.Info.LinesPerMeasurement,Data2D.Info.NumberOfMeasurements,Data2D.Info.MultiBandSliceSets,'single')*3;
Data2D.RayCount = zeros(Data2D.Info.LinesPerMeasurement,Data2D.Info.NumberOfMeasurements,Data2D.Info.MultiBandSliceSets,'single');
Data2D.PhaseRF=ones(Data2D.Info.LinesPerMeasurement,Data2D.Info.NumberOfMeasurements,Data2D.Info.MultiBandSliceSets,'single')*3;
% end adding
% adding Ye 03/29/18
Data2D.SliceInfo = FileInfo.Protocol(N).sSliceArray.asSlice;
% end adding
% Read in all 2D data
LastMeasurement=0;
for BlockIndex=1:length(FileInfo.Protocol(N).OffsetImage)
    [ComplexData,Header]=jGetDataAndHeader(FileInfo,BlockIndex);
    CurrentMeasurement=Header.Repetition+1;
    if (CurrentMeasurement>LastMeasurement)
        SliceIndex=0;
        LastMeasurement=CurrentMeasurement;
        Data2D.TriggerTime(CurrentMeasurement)=Header.Time;
    end
    if (Header.Line==0)
        SliceIndex=SliceIndex+1;
    end
    Data2D.Theta(Header.Line+1,Header.Repetition+1,SliceIndex)=Header.IceParam(5)/100;
    Data2D.PhaseRF(Header.Line+1,Header.Repetition+1,SliceIndex)=Header.IceParam(7);
    % adding Ye 03/09/18
    Data2D.LineType(Header.Line+1,Header.Repetition+1,SliceIndex) = Header.IceParam(6);
    Data2D.RayCount(Header.Line+1,Header.Repetition+1,SliceIndex) = Header.IceParam(8)+1;
    % end adding
    Kx=(1:Header.Samples)-Header.Samples+Data2D.Info.ReadoutSamples;
    Data2D.Kspace(Kx,Header.Line+1,Header.Repetition+1,SliceIndex,:)=reshape(ComplexData,Header.Samples,1,1,1,Data2D.Info.NumberOfChannels);
end
Data2D.TriggerTime=Data2D.TriggerTime-Data2D.TriggerTime(1);
