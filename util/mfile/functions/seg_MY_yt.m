...
function [seg,phi] = seg_MY_yt(I,init_mask,noi,lengthEweight,shapeEweight,display)
  ...
  %center = regionprops(init_mask,'Centroid');
  %center = center.Centroid;
  %--main loop
  for its = 1:noi   % Note: no automatic convergence test

    %idx = find(phi <= 1.2 & phi >= -1.2);  %get the curve's narrow band
    idx = find(phi <= 1.2 & phi>=-1.2);
    %[x,y] = ind2sub(size(I),idx);

    %-- find interior and exterior mean
    %upts = find(phi<=0 & phi>-2);        % interior points
    %vpts = find(phi>0 & phi<4);          % exterior points
    ...
    %F = -abs(Radial_Derivative(I,center,[x,y]));
    s = regionprops(phi<0,'Centroid');
    center = s.Centroid;
    [u,v] = get_line_average(I,idx,center);
    F = v-u;
    %F(phi(idx)<0) = 0;
