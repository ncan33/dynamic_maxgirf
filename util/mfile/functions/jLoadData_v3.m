...
if (BlockIndex<length(FileInfo.Protocol(N).OffsetImage))
    ...
    error('jLoadData_v3(): Unable to find end of first data block');
end
...
if ((SatType==SatNormal)||(SatType==SatCompressedNormal)||(SatType==SatSimultaneousNormal))
    ...
    SetStartBlock=(0:(Data.Info.NumberOfSets3D-1))*BlocksPerSet+BlocksPerAIF;
else
    BlocksPerSet=BlocksPerMeasurement/Data.Info.NumberOfSets3D;
    SetStartBlock=(0:(Data.Info.NumberOfSets3D-1))*BlocksPerSet;
end
BlockOffsetAIF=[];
