function [Image,T1_map,para] = MBR_T1_FA_1_SLG(Data,para)
...
for iT1=1:length(T1_map)
    for ifa = 1:length(fa)
        Dic(:,iT1,ifa) = Bloch_2_IR_1_SLG(para.kSpace_info.InversionTimes,0.92,T1_map(iT1),[nor,nof_one],para.kSpace_info.FlipAngle*fa(ifa),para.kSpace_info.InversionTimes(6)*1000-150,para.kSpace_info.RadialViews,para.kSpace_info.TimePerLine*1000,para.kSpace_info.TE*1000);
    end
end
Dic = Dic(:,:);
...
%     [update_term_Model,T1_map] = T1_fitting_SMS_2_SLG_pattern_recognition(LowRank_yt(new_img_x_1),Dic);
    [update_term_Model,T1_map] = T1_fitting_SMS_2_SLG_pattern_recognition(new_img_x_1,Dic);
