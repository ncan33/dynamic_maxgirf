function [Image,para] = STCR_conjugate_gradient_MSMS_systolic(Data,para)
...
%%%%% fidelity term/temporal/spatial TV
    ...
    update_term(:,:,para.sys,:,:,:) = update_term(:,:,para.sys,:,:,:) + temporal(new_img_x(:,:,para.sys,:,:,:));
    update_term(:,:,para.dia,:,:,:) = update_term(:,:,para.dia,:,:,:) + temporal(new_img_x(:,:,para.dia,:,:,:));
