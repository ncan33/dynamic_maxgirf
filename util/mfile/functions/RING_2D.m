function S = RING_2D(kSpace,theta)
%--------------------------------------------------------------------------
%   S = RING_2D(kSpace,theta)
%--------------------------------------------------------------------------
%   Estimate trajectory errors for radial k-space data
%--------------------------------------------------------------------------
%   Inputs:      
%       - kSpace    [sx, nor, nc]
%       - theta     [1,  nor]
...
for i=1:N
    ...
    %d_theta = theta_1 - theta(ray_pick-1+(2:140));
    d_theta = theta_1 - theta;
    [~,ray_pick_partner] = min(abs(abs(d_theta)-pi/2));
    %ray_pick_partner = ray_pick_partner+ray_pick;
    ...
    ray_1_im(end+1:end+sx_new*99,:) = 0;
    ray_1_im = circshift(ray_1_im,[round(sx_new*99/2),0]);
    ray_1_partner_im(end+1:end+sx_new*99,:) = 0;
    ray_1_partner_im = circshift(ray_1_partner_im,[round(sx_new*99/2),0]);
    % fft back to k-space
    ray_1_k = fft(fftshift(ray_1_im,1));
    ray_1_partener_k = fft(fftshift(ray_1_partner_im,1));
    % keep center part
    center_new = round(sx_new*100/2)+1;
