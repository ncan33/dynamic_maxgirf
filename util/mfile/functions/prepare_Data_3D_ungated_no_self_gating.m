function [Data,para] = prepare_Data_3D_ungated_no_self_gating(kSpace_radial,kSpace_info,para)
...
% 
% sys = local_max(-para.signal_self_gating);
% signal_smooth = smooth(smooth(para.signal_self_gating,20));
% sys(para.signal_self_gating(sys)>signal_smooth(sys)) = [];
% figure,plot(para.signal_self_gating)
% hold on
% plot(sys,para.signal_self_gating(sys),'o');
% hold off
% drawnow
...
% sys(sys==nof_all-3)=[];
% para.sys = sys;
% kSpace_cart =  kSpace_cart(:,:,:,sys,:) + kSpace_cart(:,:,:,sys+1,:) + kSpace_cart(:,:,:,sys+2,:);% + kSpace_cart(:,:,:,sys+3,:) + kSpace_cart(:,:,:,sys+4,:);
% mask = + mask(:,:,:,sys+3) + mask(:,:,:,sys+4) + mask(:,:,:,sys);% + mask(:,:,:,sys+1) + mask(:,:,:,sys+2);
% mask(mask==0) = 1;
...
para.kSpace_info.TimeStamp = para.kSpace_info.TimeStamp(1:nor_sl:nor_total_new);
