% --------------------------------------------------------------------------------
% Data=jLoadData_IR_T1(FileName)
% Jason Mendes - UCAIR
% --------------------------------------------------------------------------------
% Load basic data from Siemens raw data acquired with the radial flash sequence.
% --------------------------------------------------------------------------------
function Data=jLoadData_IR_T1(FileName)

% Open file and read basic data
BasicData=jLoadBasicData(FileName);
PreRecoveryMeasurement_ID=1;
RecoveryMeasurement_ID=2;
PostRecoveryMeasurement_ID=3;
ParameterWIP=zeros(1,14);
ParameterWIP(1:(length(BasicData.Protocol.sWipMemBlock.alFree)-1))=BasicData.Protocol.sWipMemBlock.alFree(2:length(BasicData.Protocol.sWipMemBlock.alFree));

% Save basic protocol information
Data=BasicData;
Data.Info.NumberOfMeasurements=BasicData.Protocol.lRepetitions+1;
Data.Info.ReadoutSamples=2*BasicData.Protocol.sKSpace.lBaseResolution;
Data.Info.NumberOfChannels=BasicData.Protocol.lNumberOfChannels;
Data.Info.FlipAngle=BasicData.Protocol.adFlipAngleDegree;
Data.Info.FOV=BasicData.Protocol.sSliceArray(1).asSlice(1).dReadoutFOV;
Data.Info.RadialViews=BasicData.Protocol.sKSpace.lRadialViews;
Data.Info.NumberOfSliceGroups=length(unique(BasicData.Slice));
Data.Info.MultiBandFactor=3;
if (BasicData.Protocol.sTXSPEC.ucRFPulseType==1)
    BasicData.Info.PulseType='Fast: Single Lobe Sinc';
else
    BasicData.Info.PulseType='Normal: 2 Lobe Asymmetric Sinc';
end
Data.Info.TR=BasicData.Protocol.alTR(1)*1e-6;
Data.Info.TE=BasicData.Protocol.alTE(1)*1e-6;
Data.Info.LinesPerMeasurement=BasicData.Protocol.sKSpace.lRadialViews;
Data.Info.TimePerLine=Data.Info.TR/(Data.Info.LinesPerMeasurement*Data.Info.NumberOfSliceGroups);
Data.Info.InversionTimes=BasicData.TimeStamp([1 find(diff(BasicData.Measurement)>0)+1]);
Data.Info.InversionTimes=Data.Info.InversionTimes-min(Data.Info.InversionTimes)+BasicData.Protocol.alTI(1)*1e-6;
Data.Info.AcquireConfiguration=sprintf('Acquire(Recover)Acquire: %d(%d)%d',ParameterWIP(PreRecoveryMeasurement_ID),ParameterWIP(RecoveryMeasurement_ID),ParameterWIP(PostRecoveryMeasurement_ID));
Data.Info.SliceThickness=BasicData.Protocol.sSliceArray.asSlice(1).dThickness;
