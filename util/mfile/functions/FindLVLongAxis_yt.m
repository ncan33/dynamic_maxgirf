...
function LV = FindLVLongAxis_yt(Image,showme)
...
[x,y] = find(BW);
notwanted = smoothed(BW)<mean(smoothed(:));
x(notwanted) = [];
y(notwanted) = [];
...
for i=1:length(x)
    ...
    %values(i,:) = values(i,:)-mean(values(i,1:10));% why
...
maxchange = max(values,[],2)-min(values,[],2);
[~,IDX] = max(maxchange);
