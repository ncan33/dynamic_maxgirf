...
tic
  ...
  while(1)
      ...
      if version > 3
          ...
          header = jsondecode(convertCharsToStrings(headerData));
	 else
          % Read header
          for k=1:hashcount
      ...
      if (version >= 2)
          ...
          % Read kspace
          %kspace(:,1:samples,i) = fread(fip, [1, samples], 'float');
      end


      if(isfield(header(1), 'dataSize'))
          dataSize = round(header(1).dataSize);
