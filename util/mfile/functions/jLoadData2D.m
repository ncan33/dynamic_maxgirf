% --------------------------------------------------------------------------------
% [Data]=jLoadData2D(FileName)
% Jason Mendes - UCAIR - University of Utah - 2017
% --------------------------------------------------------------------------------
% Load data from measurement file.
% --------------------------------------------------------------------------------
function [Data]=jLoadData2D(FileName)

% Open raw data file and initialize
fprintf('jLoadData2D(): Opening file %s\n',FileName);
FileInfo=jGetFileInfo(FileName);
Data=[];
N=length(FileInfo.Protocol);
Data.Info.Slices=FileInfo.Protocol(N).sSliceArray.lSize;
Data.Info.MultiBand=FileInfo.Protocol(N).sWipMemBlock.alFree(4);
Data.Info.MultiBandSlices=ceil(Data.Info.Slices/Data.Info.MultiBand);
Data.Info.FlipAngle=FileInfo.Protocol(N).adFlipAngleDegree;
Data.Info.ProtonDensityScans=FileInfo.Protocol(N).sWipMemBlock.alFree(5);
Data.Info.ProtonDensityFlipAngle=FileInfo.Protocol(N).sWipMemBlock.alFree(6);
Data.Info.PartitionsPerExcite=FileInfo.Protocol(N).sWipMemBlock.alFree(5);
Data.Info.RepetitionTime=FileInfo.Protocol(N).alTR(1)/1e3;
...
Data.Info.Lines=FileInfo.Protocol(N).sKSpace.lRadialViews;
...
Data.Info.MultiBandSlices = 1;
Data.Kspace(Data.Info.ReadoutSamples,Data.Info.Lines,Data.Info.MultiBandSlices,Data.Info.NumberOfChannels,Data.Info.NumberOfMeasurements)=single(complex(0,0));
Data.Theta=zeros(Data.Info.Lines,Data.Info.MultiBandSlices,Data.Info.NumberOfMeasurements,'single');
Data.LineType=zeros(Data.Info.Lines,Data.Info.MultiBandSlices,Data.Info.NumberOfMeasurements,'single');
Data.PhaseRF=zeros(Data.Info.Lines,Data.Info.MultiBandSlices,Data.Info.NumberOfMeasurements,'single');
Data.RayCount=zeros(Data.Info.Lines,Data.Info.MultiBandSlices,Data.Info.NumberOfMeasurements,'single');
for LineIndex=1:length(FileInfo.Protocol(N).OffsetImage)
    [ComplexData,Header]=jGetDataAndHeader(FileInfo,LineIndex);
    Data.Theta(Header.Line+1,Header.Slice+1,Header.Repetition+1)=Header.IceParam(5)/100;
    Data.LineType(Header.Line+1,Header.Slice+1,Header.Repetition+1)=Header.IceParam(6);
    Data.PhaseRF(Header.Line+1,Header.Slice+1,Header.Repetition+1)=Header.IceParam(7);
    Data.RayCount(Header.Line+1,Header.Slice+1,Header.Repetition+1)=Header.IceParam(8);
    Kx=(1:Header.Samples)-Header.Samples+Data.Info.ReadoutSamples;
    Data.Kspace(Kx,Header.Line+1,Header.Slice+1,:,Header.Repetition+1)=ComplexData;
end
