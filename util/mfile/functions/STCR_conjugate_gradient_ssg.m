function [Image,para] = STCR_conjugate_gradient_ssg(Data,para)
%--------------------------------------------------------------------------
...
disp('Iterative STCR conjugate gradient reconstruction...');
...
%     update_ssg = section_kernel_apply(new_img_x.*Data.sens_map, Data.ssg);
%     update_ssg = sum(update_ssg .* Data.sens_map_conj, 4);
    
    update_ssg = new_img_x.*Data.sens_map;
    update_ssg = sum(update_ssg .* Data.kernel, 4);
    update_term = update_term + (update_ssg - new_img_x) * 0.5;
