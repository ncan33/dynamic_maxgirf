function update = low_rank_yt(Image, bloc_x, bloc_y, tau)

[sx,sy,nof] = size(Image);
n_block_x = sx/bloc_x;
n_block_y = sy/bloc_y;

update = reshape(Image,[bloc_x,n_block_x,bloc_y,n_block_y,nof]);
update = permute(update,[1,3,2,4,5]);
update = reshape(update,[bloc_x*bloc_y,n_block_x*n_block_y,nof]);
...
update = reshape(update,[bloc_x,bloc_y,n_block_x,n_block_y,nof]);
update = permute(update,[1,3,2,4,5]);
update = reshape(update,[sx,sy,nof]);
end
