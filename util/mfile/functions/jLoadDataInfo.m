% --------------------------------------------------------------------------------
% [Data]=jLoadDataT1(FileInfo)
% Jason Mendes - UCAIR - University of Utah - 2016
% --------------------------------------------------------------------------------
% Load T1 data from measurement file.
% --------------------------------------------------------------------------------
function [Data]=jLoadDataInfo(FileInfo)
...
Data.Info.TR=FileInfo.Protocol(N).alTR(1)/1e3;
Data.Info.TE=FileInfo.Protocol(N).alTE(1)/1e3;
Data.Info.FlipAngle=FileInfo.Protocol(N).adFlipAngleDegree;
Data.Info.RadialViews=FileInfo.Protocol(N).sKSpace.lRadialViews;
...
Data.Info.NumberOfSlices=FileInfo.Protocol(N).sSliceArray.lSize;
Data.Info.MultiBand=FileInfo.Protocol(N).sWipMemBlock.alFree(3);
Data.Info.ProtonDensityScans=FileInfo.Protocol(N).sWipMemBlock.alFree(4);
Data.Info.ProtonDensityFlipAngle=FileInfo.Protocol(N).sWipMemBlock.alFree(5);
Data.Info.NavigatorScans=FileInfo.Protocol(N).sWipMemBlock.alFree(6);
Data.Info.CalibrationScans=FileInfo.Protocol(N).sWipMemBlock.alFree(7);
switch FileInfo.Protocol(N).sWipMemBlock.alFree(9)
    case 0
        Data.Info.SequenceType='Normal Golden Angle';
    case 1
        Data.Info.SequenceType=sprintf('%.2f * Golden Angle',1/FileInfo.Protocol(N).sWipMemBlock.alFree(10));
    case 2
        Data.Info.SequenceType=sprintf('Reset Golden Angle every %d trigger(s)',FileInfo.Protocol(N).sWipMemBlock.alFree(10));
    case 3
        Data.Info.SequenceType=sprintf('Golden Angle with Beta=pi/%d',FileInfo.Protocol(N).sWipMemBlock.alFree(10));
    otherwise
        Data.Info.SequenceType='Unknown';
end
switch FileInfo.Protocol(N).sWipMemBlock.alFree(11)
    case 0
        Data.Info.PulseType='Normal RF';
    case 1
        Data.Info.PulseType='Cycled RF';
    case 2
        Data.Info.PulseType='Optimized RF';
    otherwise
        Data.Info.PulseType='Unknown';
end
Data.Info.PostDelayTime=FileInfo.Protocol(N).sWipMemBlock.alFree(12);
Data.Info.RadialCoverage=FileInfo.Protocol(N).sWipMemBlock.alFree(13);
...
if (BlockIndex<length(FileInfo.Protocol(N).OffsetImage))
    ...
    error('jLoadDataInfo(): Unable to find end of first data block');
end
...
Ns=ceil(Data.Info.NumberOfSlices/Data.Info.MultiBand);

% Read in data
Data.Theta=zeros(Data.Info.RadialViews,Data.Info.NumberOfMeasurements,Ns,'single');
Data.LineType=zeros(Data.Info.RadialViews,Data.Info.NumberOfMeasurements,Ns,'single');
Data.PulseIndex=zeros(Data.Info.RadialViews,Data.Info.NumberOfMeasurements,Ns,'single');
Data.RayNumber=zeros(Data.Info.RadialViews,Data.Info.NumberOfMeasurements,Ns,'single');
for LineIndex=1:length(FileInfo.Protocol(N).OffsetImage)
    Header=jGetHeader(FileInfo,LineIndex);
    Data.Theta(Header.Line+1,Header.Repetition+1,Header.Slice+1)=Header.IceParam(5)/100.0;
    Data.LineType(Header.Line+1,Header.Repetition+1,Header.Slice+1)=Header.IceParam(6);
    Data.PulseIndex(Header.Line+1,Header.Repetition+1,Header.Slice+1)=Header.IceParam(7);
    Data.RayNumber(Header.Line+1,Header.Repetition+1,Header.Slice+1)=Header.IceParam(8);
end
