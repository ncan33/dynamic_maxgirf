function [Data,para] = prepare_Data_cSMS(kSpace_all,kSpace_info,para)
...
%% get sens map and G from all rays
nor_temp = floor(nor/3)*3;
rays = (1:nor_temp(1)) + sum(nor(1:1-1));
kSpace_temp = kSpace_all(:,rays,:,:);
theta_temp = theta(rays);
phase_temp = phase(rays);
for i=2:nof
    rays = (1:nor_temp(i)) + sum(nor(1:i-1));
    kSpace_temp = cat(2,kSpace_temp,kSpace_all(:,rays,:,:));
    theta_temp = cat(2,theta_temp,theta(rays));
    phase_temp = cat(2,phase_temp,phase(rays));
end
[kx,ky] = get_k_coor(sx,theta_temp,0,kCenter);
[Data.G,Data.kSpace] = GROG.GROG_seperate_SMS_GNUFFT(kSpace_temp,kx,ky,phase_temp,para);
Data.sens_map(:,:,:,:,1) = sum(Data.kSpace,7);
Data.sens_map(:,:,:,:,2) = Data.kSpace(:,:,:,:,:,:,1) + exp(-1i*2*pi/3)*Data.kSpace(:,:,:,:,:,:,2) + exp(-1i*4*pi/3)*Data.kSpace(:,:,:,:,:,:,3);
Data.sens_map(:,:,:,:,3) = Data.kSpace(:,:,:,:,:,:,1) + exp(-1i*4*pi/3)*Data.kSpace(:,:,:,:,:,:,2) + exp(-1i*2*pi/3)*Data.kSpace(:,:,:,:,:,:,3);
Data.sens_map = fftshift2(ifft2(fftshift2(Data.sens_map)));
Data.sens_map = get_sens_map(Data.sens_map,'SMS');
%% ray selection
for i=1:nof
...
switch interp_method
    ...
    case {'GROG' 'GNUFFT'}
        ...
        [Data.G,Data.kSpace] = GROG.GROG_seperate_SMS_GNUFFT(squeeze(kSpace_radial),kx,ky,phase_mod, para, Data.G);
