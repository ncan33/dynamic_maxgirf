function [Image,para] = Reconstruction_3D_ungated_single_frame(para)
...
% [Data,para] = prepare_Data_3D_low_res_ungated(kSpace,kSpace_info,para);
%[Data,para] = prepare_Data_3D_ungated_no_self_gating(kSpace,kSpace_info,para);
Image_PD = get_PD_3D_continues(kSpace,kSpace_info,para);

kSpace(:,1:kSpace_info.NumberOfPDlines*2,:) = [];
kSpace_info.angle_mod(1:kSpace_info.NumberOfPDlines*2) = [];
kSpace_info.slice_idx(1:kSpace_info.NumberOfPDlines*2) = [];
kSpace_info.NumberOfPDlines = size(kSpace,2);

Image = get_PD_3D_continues(kSpace,kSpace_info,para);
