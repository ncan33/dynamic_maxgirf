function sTV_update = compute_sTV_2_yt(img,weight,beta_sqrd)
...
if weight~=0
    ...
    T1_den = sqrt(beta_sqrd + abs(T1_num).^2 + abs(T3_num).^2);
    T3_den = sqrt(beta_sqrd + abs(T3_num).^2 + abs(T1_num).^2);
