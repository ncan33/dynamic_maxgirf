function [Image,para] = STCR_conjugate_gradient_with_ref_2(Data,para)
...
%%%%% fidelity term/temporal/spatial TV
    ...
    %ref_update = 0.1*(Data.ref - abs(new_img_x)).*phase; 
    %cost_ref = gather(sum(abs(ref_update(:))));
    update_term = fidelity_update + tTV_update + sTV_update;% + ref_update;
    clear tTV_update sTV_update slTV_update ref_update
if iter_no == 10
    %keyboard
end
%%%%% conjugate gradient
...
%%%%% line search    
    ...
    para.Cost = Cost_STCR(fidelity_update, new_img_x, weight_sTV, weight_tTV, para.Cost); clear fidelity_update
    %para.Cost.totalCost(end) = para.Cost.totalCost(end) + cost_ref;
    
    step_size = line_search_with_ref_2(new_img_x,update_term_old,Data,para);
