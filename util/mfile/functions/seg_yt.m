...
function [seg,phi] = seg_yt(I,init_mask,noi,lengthEweight,shapeEweight,display)
  ...
  for its = 1:noi   % Note: no automatic convergence test
    ...
    dphidt = F./max(abs(F))*2 + lengthEweight*curvature + shapeEweight*dEdl;  % gradient descent to minimize energy
