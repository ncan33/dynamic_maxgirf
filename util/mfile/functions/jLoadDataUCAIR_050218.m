...
if ((SequenceType==Sequence_2D)||(SequenceType==Sequence_2D_3D))
    ...
    if (SequenceType==Sequence_2D_3D)
        ...
        Data2D.Info.NumberOfContrasts=1;
    else
...
if ((contains(AlignKspaceCenter,'Calculate','IgnoreCase',true))&&(~isempty(Location2D)))
    ...
    if (N>10)
        ...
        if (abs(Data2D.Info.AlignPixel.AppliedShift)>3)
            fprintf('jLoadDataUCAIR(): Pixel shift for 2D data is too large.\n');    
        end
        if (contains(AlignKspaceCenter,'Apply','IgnoreCase',true)&&(abs(Data2D.Info.AlignPixel.AppliedShift)>3))
            DataMask=zeros(size(Data2D.Kspace),'single');
            if (Data2D.Info.AlignPixel.AppliedShift<0)
                DataMask((min(Kx)-floor(Data2D.Info.AlignPixel.AppliedShift)):max(Kx),:,:,:)=1;
            else
                DataMask(max(min(Kx)-floor(Data2D.Info.AlignPixel.AppliedShift),1):(max(Kx)-ceil(Data2D.Info.AlignPixel.AppliedShift)),:,:,:)=1;
            end
            ...
            Data2D.Kspace=DataMask.*fftshift(ifft(ifftshift(fftshift(fft(ifftshift(Data2D.Kspace,1),[],1),1).*exp(Data2D.Info.AlignPixel.AppliedShift*ShiftMatrix),1),[],1),1);
        end
    end
end
...
if ((contains(AlignKspaceCenter,'Calculate','IgnoreCase',true))&&(~isempty(Location3D)))
    ...
    if (N>10)
        ...
        if (abs(Data3D.Info.AlignPixel.AppliedShift)>3)
            fprintf('jLoadDataUCAIR(): Pixel shift for 3D data is too large.\n');    
        end
        if (contains(AlignKspaceCenter,'Apply','IgnoreCase',true)&&(abs(Data3D.Info.AlignPixel.AppliedShift)>3))
...
if (~isempty(LocationAIF))
    ...
    DataAIF.Set=reshape(SliceSet(LocationAIF(1:DataAIF.Info.NumberOfContrasts:length(LocationAIF))),1,NumberOfLines);
...
if ((contains(AlignKspaceCenter,'Calculate','IgnoreCase',true))&&(~isempty(Location3D)))
    ...
    if (N>10)
        ...
        if (abs(DataAIF.Info.AlignPixel.AppliedShift)>3)
            fprintf('jLoadDataUCAIR(): Pixel shift for AIF data is too large.\n');    
        end
        if (contains(AlignKspaceCenter,'Apply','IgnoreCase',true)&&(abs(DataAIF.Info.AlignPixel.AppliedShift)>3))
