function step = line_search_TFD_l1_l2(old, update, Data, para)
%--------------------------------------------------------------------------
...
%      fprintf(['Iter = ' num2str(i) '... '])
    ...
    switch para.Recon.type
        case {'3D','3D SOS','3D SOS server'}
            cost_new = Cost_STCR_3D(fidelity_new, new, para.Recon.weight_l2, para.Recon.weight_tTV, para.Recon.weight_sliceTV);
        otherwise
            cost_new = Cost_TFD_l1_l2(fidelity_new, new, para.Recon.weight_l2, para.Recon.weight_tTV);
    end
