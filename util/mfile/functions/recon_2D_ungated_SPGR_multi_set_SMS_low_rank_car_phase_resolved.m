function recon_2D_ungated_SPGR_multi_set_SMS_low_rank_car_phase_resolved(kSpace_all,kSpace_info,para)
...
Image_PD = recon_PD(kSpace_all(:,1:kSpace_info.NumberOfPDlines,:),kSpace_info,para);
...
cardiac_signal_mask = std(cardiac_signal(:,:,1:round(size(Image,3)/2),:,:),1,3);
cardiac_signal = (cardiac_signal_mask > max(cardiac_signal_mask(:))*0.5).*cardiac_signal;
cardiac_signal = squeeze(sum(sum(sum(cardiac_signal(:,:,:,:),4))));

cardiac_signal = smooth(cardiac_signal);
...
im_bin = STCR_conjugate_gradient_MSMS_low_rank_bins(Data_all,para);
