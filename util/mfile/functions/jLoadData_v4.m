...
% --------------------------------------------------------------------------------
function [Data]=jLoadData_v4(FileInfo)
...
Data.Info.NumberOfSetsAIF=(TotalSets-Data.Info.NumberOfSets3D)/FileInfo.Protocol(N).sWipMemBlock.alFree(8);
...
if (BlockIndex<length(FileInfo.Protocol(N).OffsetImage))
    ...
    error('jLoadData_v4(): Unable to find end of first data block');
end
...
for SetIndex=1:length(BlockOffsetAIF)
    ...
    for MeasIndex=1:Data.Info.NumberOfMeasurements
        for BlockIndex=1:BlocksPerAIF
            ...
            Header.Echo=Header.Set-BaseSet;
