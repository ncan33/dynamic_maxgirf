function llr = Patch_tracking_4D_with_guide(Data,para,mask,offset_init,patch_size,search_size,patch_shift)
...
if ~exist('patch_size')
    patch_size = [5,5,2,Nphase];
end
if ~exist('search_size')
    search_size = [5,5,2,Nphase];
end
...
if x_begin_all(end) + patch_size(1)-1 < sx
    x_begin_all(end+1) = nslice-patch_size(1) + 1;
end
if y_begin_all(end) + patch_size(2)-1 < sy
    y_begin_all(end+1) = nslice-patch_size(2) + 1;
end
