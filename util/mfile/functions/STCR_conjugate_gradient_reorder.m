function [Image,para] = STCR_conjugate_gradient_reorder(Data,para)
...
temporal = @(im) compute_tTV_reorder(im,Data.order,Data.order_back,weight_tTV,beta_sqrd);
...
%     update_term = update_term + compute_tTV_random(new_img_x,weight_tTV*0.2,beta_sqrd);
...
%%%%% line search    
    ...
    para.Cost = Cost_STCR(fidelity_norm, new_img_x(Data.order), weight_sTV, weight_tTV, para.Cost); clear fidelity_update
    step_size = line_search_reorder(new_img_x,update_term_old,Data,para);
