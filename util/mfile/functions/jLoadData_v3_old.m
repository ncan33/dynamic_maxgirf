...
% --------------------------------------------------------------------------------
function [Data]=jLoadData_v3(FileInfo)
...
SatCompressed=1;
SatSimultaneous=2;
SatCompressedNormal=3;
SatSimultaneousNormal=4;
Data=[];
N=length(FileInfo.Protocol);
TotalSets=FileInfo.Protocol(N).NSetMeas;
Data.Info.NumberOfSets3D=FileInfo.Protocol(N).sWipMemBlock.alFree(2);
Data.Info.NumberOfSetsAIF=TotalSets-Data.Info.NumberOfSets3D;
...
switch SatType
    case SatNormal
        Data.Info.SequenceType='Normal: AIF before 3D';
    case SatCompressed
        Data.Info.SequenceType='Compressed: AIF during 3D SRT';
    case SatSimultaneous
        Data.Info.SequenceType='Simultaneous: AIF in 3D volume';
    case SatCompressedNormal
        Data.Info.SequenceType='Compressed+Normal: AIF before 3D and during 3D SRT';
    case SatSimultaneousNormal
        Data.Info.SequenceType='Simultaneous+Normal: AIF before 3D and in 3D volume';
    otherwise
        Data.Info.SequenceType='Unknown';
end
if (length(FileInfo.Protocol(N).sWipMemBlock.alFree)>14)
    Data.Info.PositionAIF=FileInfo.Protocol(N).sWipMemBlock.alFree(15);
end
%Data.Info.RepetitionTime3D=
...
BlocksPerMeasurement=length(FileInfo.Protocol(N).OffsetImage)/Data.Info.NumberOfMeasurements;
...
if ((SatType==SatNormal)||(SatType==SatCompressedNormal)||(SatType==SatSimultaneousNormal))
    BlocksPerSet=(BlocksPerMeasurement-BlocksPerAIF)/Data.Info.NumberOfSets3D;
    SetStartBlock=(0:(TotalSets-2))*BlocksPerSet+BlocksPerAIF;
else
    BlocksPerSet=BlocksPerMeasurement/Data.Info.NumberOfSets3D;
    SetStartBlock=(0:(TotalSets-1))*BlocksPerSet;
end
BlockOffsetAIF=[];
BlockOffset3D=SetStartBlock;
...
if ((SatType==SatNormal)||(SatType==SatCompressedNormal)||(SatType==SatSimultaneousNormal))
    BlockOffsetAIF=[BlockOffsetAIF 0];
end
if ((SatType==SatCompressedNormal)||(SatType==SatCompressed))
    for SetIndex=1:Data.Info.NumberOfSets3D
        BlockOffsetAIF=[BlockOffsetAIF SetStartBlock(SetIndex)];
    end
    BlockOffset3D=BlockOffset3D+BlocksPerAIF;
end

% Read in AIF data
for SetIndex=1:length(BlockOffsetAIF)
    CurrentData=single(complex(0,0));
    CurrentData(Data.Info.NumberOfLinesAIF,Data.Info.ReadoutSamples,1,Data.Info.NumberOfChannels,Data.Info.NumberOfMeasurements,Data.Info.NumberOfContrastsAIF)=single(complex(0,0));
    CurrentData(:)=single(complex(0,0));
    ...
    for MeasIndex=1:Data.Info.NumberOfMeasurements
        for BlockIndex=1:BlocksPerAIF
            ...
            if (rem(Header.Echo,2))
                CurrentData(Ky,Kx,1,:,MeasIndex,Header.Echo+1)=ComplexData(Header.Samples:-1:1,:,:,:);
            else
                CurrentData(Ky,Kx,1,:,MeasIndex,Header.Echo+1)=ComplexData;
            end
        end
    end
    eval(sprintf('Data.AIF_%s=permute(CurrentData,[2 1 3 5 6 4]);',BlockNameAIF(min(SetIndex,length(BlockNameAIF))).String));
...
for SetIndex=1:length(BlockOffset3D)
    CurrentData=single(complex(0,0));
    CurrentData(Data.Info.LinesInCentralPartition,Data.Info.ReadoutSamples,Data.Info.Partitions3D,Data.Info.NumberOfChannels,Data.Info.NumberOfMeasurements)=single(complex(0,0));
    CurrentData(:)=single(complex(0,0));
    ...
    for MeasIndex=1:Data.Info.NumberOfMeasurements
        for BlockIndex=1:Data.Info.NumberOfLines3D
            ...
            Ky=Header.Line+1;
            Kz=Header.Partition-Header.KspacePartitionCenter+floor(Nz/2)+1;
            Kx=(1:Header.Samples)-Header.Samples+Data.Info.ReadoutSamples;
            Theta(MeasIndex,Ky,Kz)=Header.IceParam(5)/100;
            CurrentData(Ky,Kx,Kz,:,MeasIndex)=ComplexData;
        end
    end
    eval(sprintf('Data.3D_%s=permute(CurrentData,[2 1 3 5 4]);',BlockName3D(min(SetIndex,length(BlockName3D))).String));
    eval(sprintf('Data.3D_%s_Theta=Theta;',BlockName3D(min(SetIndex,length(BlockName3D))).String));
    fprintf('Done\n');
end
