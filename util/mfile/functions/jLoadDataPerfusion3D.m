...
% Jason Mendes - UCAIR - University of Utah
% --------------------------------------------------------------------------------

% --------------------------------------------------------------------------------
function [Data3D,DataAIF]=jLoadDataPerfusion3D(FileName)

fprintf('--------------------------------------------------------------------------------\n');
fprintf('jLoadDataPerfusion3D() - Jason Mendes - UCAIR - University of Utah\n')
fprintf('--------------------------------------------------------------------------------\n');

% Definitions from sequence (UCAIR_Definitions.h)
UCAIR_SEQUENCE_2D=0;
UCAIR_SEQUENCE_3D=1;
UCAIR_SEQUENCE_2D_PD=65534;
UCAIR_SEQUENCE_3D_PD=65535;
...
DataAIF=[];
tic;
BasicData=jLoadBasicData(FileName);
Data.Info=jGetBasicProtocol(BasicData,'Perfusion 3D');
DataLocationAIF=find((BasicData.LineType==UCAIR_SEQUENCE_2D)|(BasicData.LineType==UCAIR_SEQUENCE_2D_PD));
DataLocation3D=find((BasicData.LineType==UCAIR_SEQUENCE_3D)|(BasicData.LineType==UCAIR_SEQUENCE_3D_PD));

% Format AIF data
fprintf('jLoadDataPerfusion3D(): Loading AIF protocol\n');
DataAIF=Data;
DataAIF.Info.TimePerLine_ms=Data.Info.SiemensHeader.alTR(3);
DataAIF.Info.NumberOfContrasts=Data.Info.SiemensHeader.lContrasts;
DataAIF.Info.EchoTime_ms=Data.Info.SiemensHeader.alTE((1:DataAIF.Info.NumberOfContrasts)+1);
%DataAIF.Info.SliceThickness_mm=ProtocolData.SiemensHeader.sWipMemBlock.alFree(4);
% This and below, Ye 11/16/18
DataAIF.Info.SliceThickness_mm=DataAIF.Info.SiemensHeader.sWipMemBlock.alFree(4);
%DataAIF.Info.RecoveryTime_ms=ProtocolData.SiemensHeader.sWipMemBlock.alFree(5);
DataAIF.Info.RecoveryTime_ms=DataAIF.Info.SiemensHeader.sWipMemBlock.alFree(5);
DataAIF.Info.NumberOfSets=length(unique(BasicData.Set(DataLocationAIF)));

if ((DataAIF.Info.NumberOfSets*DataAIF.Info.RadialViews*DataAIF.Info.NumberOfMeasurements*DataAIF.Info.NumberOfContrasts)~=length(DataLocationAIF))
    error('jLoadDataPerfusion3D(): AIF data has wrong number of lines\n');
end
DataAIF.Kspace=permute(reshape(BasicData.Kspace(:,DataLocationAIF,1,:),DataAIF.Info.ReadoutSamples,DataAIF.Info.NumberOfContrasts,...
    DataAIF.Info.RadialViews*DataAIF.Info.NumberOfSets*DataAIF.Info.NumberOfMeasurements,Data.Info.NumberOfChannels),[1 3 2 4]);
DataAIF.TimeStamp=min(reshape(BasicData.TimeStamp(DataLocationAIF),DataAIF.Info.NumberOfContrasts*DataAIF.Info.RadialViews*DataAIF.Info.NumberOfSets,...
    DataAIF.Info.NumberOfMeasurements),[],1);
DataAIF.TimeStamp=DataAIF.TimeStamp-DataAIF.TimeStamp(1);
DataAIF.Theta=reshape(permute(reshape(BasicData.Theta(DataLocationAIF),DataAIF.Info.NumberOfContrasts,...
    DataAIF.Info.RadialViews*DataAIF.Info.NumberOfSets*DataAIF.Info.NumberOfMeasurements),[2 1]),1,length(DataLocationAIF));
DataAIF.Set=reshape(permute(reshape(BasicData.Set(DataLocationAIF),DataAIF.Info.NumberOfContrasts,...
    DataAIF.Info.RadialViews*DataAIF.Info.NumberOfSets*DataAIF.Info.NumberOfMeasurements),[2 1]),1,length(DataLocationAIF));
SetMap=unique(DataAIF.Set);
SetIndex=zeros(1,max(SetMap)+1);
SetIndex(SetMap+1)=0:(DataAIF.Info.NumberOfSets-1);
DataAIF.Set=SetIndex(DataAIF.Set+1);
DataAIF.Measurement=reshape(permute(reshape(BasicData.Measurement(DataLocationAIF),DataAIF.Info.NumberOfContrasts,...
    DataAIF.Info.RadialViews*DataAIF.Info.NumberOfSets*DataAIF.Info.NumberOfMeasurements),[2 1]),1,length(DataLocationAIF));

% Align AIF data
fprintf('jLoadDataPerfusion3D(): Align AIF data\n');
AlignIndex=1:length(DataAIF.Measurement);
N=length(AlignIndex);
Kx=find(DataAIF.Kspace(:,1,1,1)>(2*realmin('single')));
...
DataAIF.Info.AlignPixel.Offset=-2:0.2:2;
...
DataMask=zeros(size(DataAIF.Kspace),'single');
if (DataAIF.Info.AlignPixel.AppliedShift<0)
    DataMask((min(Kx)-floor(DataAIF.Info.AlignPixel.AppliedShift)):max(Kx),:,:,:)=1;
else
    DataMask(max(min(Kx)-floor(DataAIF.Info.AlignPixel.AppliedShift),1):(max(Kx)-ceil(DataAIF.Info.AlignPixel.AppliedShift)),:,:,:)=1;
end
...
DataAIF.Kspace=DataMask.*fftshift(ifft(ifftshift(fftshift(fft(ifftshift(DataAIF.Kspace,1),[],1),1).*exp(DataAIF.Info.AlignPixel.AppliedShift*ShiftMatrix),1),[],1),1);

% Format 3D data
fprintf('jLoadDataPerfusion3D(): Format 3D data\n');
Data3D=Data;
Data3D.Info.TimePerLine_ms=TR(3);
Data3D.Info.NumberOfContrasts=1;
Data3D.Info.EchoTime_ms=TE(1);
Data3D.Info.NumberOfSets=ParameterWIP(1);
Data3D.Info.SliceThickness_mm=Data.Protocol.sSliceArray.asSlice(1).dThickness;
Data3D.Info.RecoveryTime_ms=Data.Protocol.alTI(1)*1e-3;
Data3D.Info.NumberOfPartitions=Data.Protocol.sKSpace.lPartitions;
Data3D.Info.LinesPerMeasurement=ceil(length(DataLocation3D)/Data3D.Info.NumberOfMeasurements/Data3D.Info.NumberOfSets);
if ((Data3D.Info.NumberOfSets*Data3D.Info.LinesPerMeasurement*Data3D.Info.NumberOfMeasurements)~=length(DataLocation3D))
    error('jLoadDataPerfusion3D(): 3D data has wrong number of lines\n');
end
Data3D.Kspace=BasicData.Kspace(:,DataLocation3D,1,:);
Data3D.TimeStamp=min(reshape(BasicData.TimeStamp(DataLocation3D),Data3D.Info.LinesPerMeasurement*Data3D.Info.NumberOfSets,...
    Data3D.Info.NumberOfMeasurements),[],1);
Data3D.TimeStamp=Data3D.TimeStamp-Data3D.TimeStamp(1);
Data3D.Theta=BasicData.Theta(DataLocation3D);
Data3D.Set=BasicData.Set(DataLocation3D);
SetMap=unique(Data3D.Set);
SetIndex=zeros(1,max(SetMap)+1);
SetIndex(SetMap+1)=0:(Data3D.Info.NumberOfSets-1);
Data3D.Set=SetIndex(Data3D.Set+1);
Data3D.Measurement=BasicData.Measurement(DataLocation3D);
Data3D.Partition=BasicData.Partition(DataLocation3D)+floor(Data3D.Info.NumberOfPartitions/2)+1;

% Align 3D data
fprintf('jLoadDataUCAIR(): Align 3D data\n');
AlignIndex=find((Data3D.Partition==(floor(Data3D.Info.NumberOfPartitions/2)+1)));
N=length(AlignIndex);
...
Data3D.Info.AlignPixel.Offset=-2:0.2:2;
...
DataMask=zeros(size(Data3D.Kspace),'single');
if (Data3D.Info.AlignPixel.AppliedShift<0)
    DataMask((min(Kx)-floor(Data3D.Info.AlignPixel.AppliedShift)):max(Kx),:,:,:)=1;
else
    DataMask(max(min(Kx)-floor(Data3D.Info.AlignPixel.AppliedShift),1):(max(Kx)-ceil(Data3D.Info.AlignPixel.AppliedShift)),:,:,:)=1;
end
...
Data3D.Kspace=DataMask.*fftshift(ifft(ifftshift(fftshift(fft(ifftshift(Data3D.Kspace,1),[],1),1).*exp(Data3D.Info.AlignPixel.AppliedShift*ShiftMatrix),1),[],1),1);


% --------------------------------------------------------------------------------
% Data=jLoadBasicData(FileName)
% Jason Mendes - UCAIR
% --------------------------------------------------------------------------------
% Load basic data from Siemens raw data acquired with the radial flash sequence.
% --------------------------------------------------------------------------------
function Data=jLoadBasicData(FileName)

% Open file and initialize
FileInfo=jGetFileInfo(FileName);
Data=[];
Data.Protocol=FileInfo.Protocol(length(FileInfo.Protocol));
NumberOfLines=length(Data.Protocol.OffsetImage);
ReadoutSamples=2*Data.Protocol.sKSpace.lBaseResolution;
NumberOfChannels=Data.Protocol.lNumberOfChannels;

% Read all data and needed header information
Data.Measurement=zeros(1,NumberOfLines,'single');
Data.Theta=zeros(1,NumberOfLines,'single');
Data.LineType=zeros(1,NumberOfLines,'single');
Data.PhaseCycle=zeros(1,NumberOfLines,'single');
Data.LineCount=zeros(1,NumberOfLines,'single');
Data.Group=zeros(1,NumberOfLines,'single');
Data.Set=zeros(1,NumberOfLines,'single');
Data.Slice=zeros(1,NumberOfLines,'single');
Data.Contrast=zeros(1,NumberOfLines,'single');
Data.Partition=zeros(1,NumberOfLines,'single');
Data.TimeStamp=zeros(1,NumberOfLines,'single');
Data.PMU=zeros(1,NumberOfLines,'single');
Data.Kspace=complex(0,zeros(ReadoutSamples,NumberOfLines,1,NumberOfChannels,'single'));
for LineIndex=1:NumberOfLines
    [ComplexData,Header]=jGetDataAndHeader(FileInfo,LineIndex);
    Kx=(1:Header.Samples)-Header.Samples+ReadoutSamples;
    if (Header.Reflect)
        Data.Kspace(Kx,LineIndex,1,:)=ComplexData(Header.Samples:-1:1,1,1,:);
    else
        Data.Kspace(Kx,LineIndex,1,:)=ComplexData;
    end
    Data.Measurement(LineIndex)=Header.Repetition;
    Data.Theta(LineIndex)=Header.IceParam(5)/100;
    Data.LineType(LineIndex)=Header.IceParam(6);
    Data.PhaseCycle(LineIndex)=Header.IceParam(7);
    Data.LineCount(LineIndex)=Header.IceParam(8);
    Data.Group(LineIndex)=Header.IceParam(1);
    Data.Set(LineIndex)=Header.IceParam(2);
    Data.Slice(LineIndex)=Header.IceParam(3);
    Data.Contrast(LineIndex)=Header.IceParam(4);
    Data.Partition(LineIndex)=Header.Partition-Header.KspacePartitionCenter;
    Data.TimeStamp(LineIndex)=Header.Time;
    Data.PMU(LineIndex)=Header.Pmu;
end
