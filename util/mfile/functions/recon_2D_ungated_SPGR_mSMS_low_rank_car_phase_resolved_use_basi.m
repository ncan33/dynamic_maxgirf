function recon_2D_ungated_SPGR_mSMS_low_rank_car_phase_resolved_use_basis(kSpace_all,kSpace_info,para)
...
%% estimate basis form low res image
for i=1:nset
    ...
    Data{i}.kSpace = crop_half_FOV(Data{i}.kSpace);
...
for i=1:nset
    for j=1:para.Recon.nSMS
        Image_temp = Image(:,:,:,i,j);
        [sx,sy,nof] = size(Image_temp);
        Image_temp = reshape(Image_temp,sx*sy,nof);
        [u,s,v] = svd(Image_temp.');
        Data{i}.basis{j} = u(:,1:10);
    end
end

%% pre-interpolate for high resolution image
...
for i=1:nset
    ...
    [Data{i}.kSpace,Data{i}.G] = GROG.SMS_GROG(kSpace_radial,kx,ky,phase,para);
    Data{i}.kSpace = Data{i}.kSpace;
    
    para.Recon.nor = nor_sl;
    Data{i} = rmfield(Data{i},'sens_map');
    [Data{i},para] = get_Data_SMS(Data{i},para);
...
for i=1:nset
    ...
    Data{i}.kSpace = kSpace;
    Image(:,:,:,i,:) = STCR_conjugate_gradient_use_basis(Data{i},para);
    clear kSpace
end
...
order = 1:nset;
for i=nSMS:-1:2
    order = [order,(i-1)*(nset)+1:i*nset];
end
% [~,order_back] = sort(order);
for i=1:nslice
    mask(:,:,i) = get_mask(abs(Image(:,:,:,order(i))));
end

cardiac_signal = abs(Image(:,:,:,order)).*permute(mask,[1,2,4,3]);
cardiac_signal = squeeze(sum(sum(sum(cardiac_signal)),4));
sys = local_max(-cardiac_signal);
para.Recon.self_gating.cardiac_signal = cardiac_signal;
para.Recon.self_gating.sys = sys;
figure
...
keyboard 
para.Recon.cardiac_signal = cardiac_signal;
para.Recon.self_gating.sys = sys;
% reshape k-space
...
%
if para.load_flag
    load('data.mat','mask','shifts_interp')
else
    [~,shifts_dia,~] = rigid_reg_GS_3D(Image_dia,crop_half_FOV(Data_all.myo_mask));
    ...
    shifts_interp = interp_rigid_shifts(shifts_dia,resp_signal,para.Recon.bins(dia_loc,:));
...
para.setting.ifGPU = 1;
para.Recon.noi = 50;
im_bin = STCR_conjugate_gradient_MSMS_tv(Data_all,para);
