...
ArterialInputMode_ID=7;
GateMode_ID=8;
SaturationMode_ID=9;
ProtonDensityScans_ID=10;
ProtonDensityFlipAngle_ID=11;
MultiBandFactor_ID=12;
PulseDuration2D_ID=13;
Sequence_2D=0;
Sequence_3D=1;
Sequence_2D_3D=2;
Sequence_AIF=3;
...
Info.ProtonDensityScans=ParameterWIP(ProtonDensityScans_ID);
Info.ProtonDensityFlipAngle=ParameterWIP(ProtonDensityFlipAngle_ID);
...
if ((SequenceType==Sequence_3D)||(SequenceType==Sequence_2D_3D))
    ...
    elseif (ParameterWIP(ArterialInputMode_ID)==AIF_Both)
        DataAIF.Info.MeasuredAIF='Both: Before and during SRT';
    elseif (ParameterWIP(ArterialInputMode_ID)==AIF_Efficient)
...
if (~isempty(Location2D))
    ...
    % added Ye 04/12/18
    Data2D.LineType = zeros(1,NumberOfLines,'single'); 
    %
    for LineIndex=1:NumberOfLines
        for ContrastIndex=1:Data2D.Info.NumberOfContrasts
            ...
            if (ContrastIndex==1)
                ...
                % added Ye 04/12/18
                Data2D.LineType(LineIndex) = Header.IceParam(6);
                %
            end
...
if 0 %((contains(AlignKspaceCenter,'Calculate','IgnoreCase',true))&&(~isempty(Location2D)))
...
if (~isempty(Location3D))
    ...
    % added Ye 04/12/18
    Data3D.LineType = zeros(1,NumberOfLines,'single'); 
    %
    for LineIndex=1:NumberOfLines
        ...
        % added Ye 04/12/18
        Data3D.LineType(LineIndex) = Header.IceParam(6);
        %
    end
...
if (~isempty(LocationAIF))
    ...
    % added Ye 04/12/18
    DataAIF.LineType = zeros(1,NumberOfLines,'single'); 
    %
    for LineIndex=1:NumberOfLines
        for ContrastIndex=1:DataAIF.Info.NumberOfContrasts
            ...
            if (ContrastIndex==1)
                ...
                % added Ye 04/12/18
                DataAIF.LineType(LineIndex) = Header.IceParam(6);
                %
            end
