function [Image,para] = iteration_recon_update_sens(first_est,kSpace,sens_map,phase_mod,para,varargin)
...
%    tic
    disp([num2str(round(iter_no/(noi_end)*100)),'%...']);

%%%%% fidelity term/temporal/spatial TV
if mod(iter_no,10)==0
    tic; [fidelity_update,sens_map] = compute_fidelity_sens_yt(ifNUFFT,nSMS,'kspace',new_img_x,kSpace,sens_map,conj(sens_map),phase_mod,phase_mod_conj,varargin);para.CPUtime.fidelity(iter_no) = toc;
else
    tic; fidelity_update = compute_fidelity_yt(ifNUFFT,nSMS,'kspace',new_img_x,kSpace,sens_map,conj(sens_map),phase_mod,phase_mod_conj,varargin);para.CPUtime.fidelity(iter_no) = toc;
end
