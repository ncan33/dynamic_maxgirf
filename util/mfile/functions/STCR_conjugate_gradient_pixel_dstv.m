function [Image,para] = STCR_conjugate_gradient_pixel_dstv(Data,para)
...
%%%%% fidelity term/temporal/spatial TV
    ...
    tic; %tTV_update = compute_tTV_yt(new_img_x,weight_tTV,beta_sqrd);
    tTV_update = compute_tTV_pixel(new_img_x,weight_tTV,beta_sqrd,Data.Motion);  para.CPUtime.tTV(iter_no) = toc;

    %tTV_update(73:216,73:216,:) = tTV_update_center;
    %tTV_update(1:72,:,:) = 0;
    %tTV_update(:,1:72,:) = 0;
    ...
    %tic; sTV_update = compute_sTV_yt(new_img_x,weight_sTV,beta_sqrd).*Data.edge;  para.CPUtime.sTV(iter_no) = toc;
    %tic; sTV_update = compute_dsTV_yt(new_img_x,Data.phi,weight_sTV,beta_sqrd);  para.CPUtime.sTV(iter_no) = toc;
tic;sTV_update = compute_sTV_yt(new_img_x,weight_sTV,beta_sqrd).*Data.edge + compute_dsTV_yt(new_img_x,Data.phi,weight_sTV,beta_sqrd).*(1-Data.edge);para.CPUtime.sTV(iter_no) = toc;
...
%%%%% line search    
    ...
    para.Cost = Cost_STCR_pixel_dstv(fidelity_update, new_img_x, Data, weight_sTV, weight_tTV, para.Cost); clear fidelity_update
    step_size = line_search_pixel_dstv(new_img_x,update_term_old,Data,para);
