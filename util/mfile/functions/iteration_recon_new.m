function [Image,para] = iteration_recon_new(first_est,kSpace,sens_map,phase_mod,para,varargin)
...
%%%%% fidelity term/temporal/spatial TV
    ...
    %fidelity_update(:,:,:,:,1) = fft2(fidelity_update(:,:,:,:,1));
    %fidelity_update(:,:,:,:,2) = fft2(fidelity_update(:,:,:,:,2));
    %fidelity_update(:,:,:,:,3) = fft2(fidelity_update(:,:,:,:,3));
    ...
    %fidelity_update(:,:,:,:,1) = ifft2(fidelity_update(:,:,:,:,1));
    %fidelity_update(:,:,:,:,2) = ifft2(fidelity_update(:,:,:,:,2));
    %fidelity_update(:,:,:,:,3) = ifft2(fidelity_update(:,:,:,:,3));
