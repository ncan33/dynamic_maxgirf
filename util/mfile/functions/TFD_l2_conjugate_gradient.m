function [Image,para] = TFD_l2_conjugate_gradient(Data,para)
%--------------------------------------------------------------------------
...
%% update term (gradient of fidelity, temporal/spatial TV)
    ...
    tic;
    update_term = update_term + circshift(TFD(TFD(new_img_x)), [0, 0, 1]);
...
%% line search    
    ...
    para.Cost = Cost_TFD_l2(fidelity_norm, new_img_x, weight_sTV, weight_tTV, para.Cost); clear fidelity_update
    step_size = line_search_TFD_l2(new_img_x,update_term_old,Data,para);
