...
%% size
nsample         = size(kspace, 2);
ncoil           = size(kspace, 3);

%% split kspace
kspace_echo_1 = kspace(:, echo_idx == 1, :);
kspace_echo_1 = permute(kspace_echo_1, [2,1,3]);
kspace_echo_2 = kspace(:, echo_idx == 2, :);
kspace_echo_2 = permute(kspace_echo_2, [2,1,3]);
%clear kspace
...
nsample_1 = size(kspace_echo_1, 1);
nsample_2 = size(kspace_echo_2, 1);

kspace_echo_1  = reshape(kspace_echo_1, [nsample_1, narm_frame, nframes, ncoil]);
kspace_echo_2  = reshape(kspace_echo_2, [nsample_2, narm_frame, nframes, ncoil]);
