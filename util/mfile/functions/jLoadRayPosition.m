% --------------------------------------------------------------------------------
% RayPosition=jLoadRayPosition(FileName)
% Jason Mendes - UCAIR - University of Utah - 2015
% --------------------------------------------------------------------------------
% Load ray position and phase order from measurement file.
% --------------------------------------------------------------------------------
function [RayPosition,PulsePhase]=jLoadRayPosition(FileName)

% Prepare to read the data
FileInfo=jGetFileInfo(FileName);
N=length(FileInfo.Protocol);
Nr=FileInfo.Protocol(N).sKSpace.lRadialViews;
Nm=FileInfo.Protocol(N).lRepetitions+1;
Nb=length(FileInfo.Protocol(N).OffsetImage);
RayPosition=zeros(Nr,Nm);
LineCounter=zeros(Nr,Nm);
PulseType=zeros(Nr,Nm);
PulsePhase=zeros(Nr,Nm);
TimeSinceTrigger=zeros(Nr,Nm);
LastTime=1e10;
RayCount=0;
...
for LineIndex=1:Nb
    ...
    RayIndex=Header.Line+1;
    MeasIndex=Header.Repetition+1;
    TimeSinceTrigger(RayIndex,MeasIndex)=Header.Pmu;
    if (Header.Pmu<LastTime)
        RayCount=0;
    end
    RayCount=RayCount+1;
    RayPosition(RayIndex,MeasIndex)=RayCount;
    LastTime=Header.Pmu;
    LineCounter(RayIndex,MeasIndex)=Header.IceParam(8);
    PulseType(RayIndex,MeasIndex)=Header.IceParam(7);
    PulsePhase(RayIndex,MeasIndex)=Header.IceParam(6);
end
